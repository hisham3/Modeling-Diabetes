...
def fitting(x,y):
    c, cov= curve_fit(eq_solving(x,y), x, y, maxfev=5000)
...
def plot(x_data1, x_data2,x_subs, y_data1, y_data2, y_subs1, y_subs2, diabetic):
    ...
    if diabetic == 'Has Diabetes':
        plt.plot(x_subs, y_subs2, label=f'Your Data ({diabetic})')
        plt.scatter(x_data2, y_data2, marker='o', alpha=0.3)
    else:
        plt.plot(x_subs, y_subs2, label=f'Your Data ({diabetic})', color='green')
        plt.scatter(x_data2, y_data2, marker='o', c='green', alpha=0.3)
